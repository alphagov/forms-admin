class Pages::ChangeOrderInput < BaseInput
  ...
  def update_preview
    ...
    @pages = change_ordered_page_ids.map { form.pages.find(it) }
  ...
  def pages
    ...
    page_position_params.to_h.map { |name, _value| form.pages.find(page_id_from_input_name(name)) }
  end
  ...
  def validate_positions
    page_position_params.each do |name, value|
      ...
      if !NUMERIC_PATTERN.match?(value) || value.to_i < 1 || value.to_i > maximum
        errors.add(name.to_sym, I18n.t("activemodel.errors.models.pages/change_order_input.attributes.page_position.invalid", maximum:))
      end
    end
  end
