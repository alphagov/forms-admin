class Forms::SubmissionEmailForm
  ...
  validates :temporary_submission_email, presence: true, if: -> { form.live? }
  ...
  def assign_form_values
    # Need to handle when the record doesn't exist
    # return self if FormSubmissionEmail.find_by!(form_id: form.id).nil?
    form_submission_email = FormSubmissionEmail.find_by_form_id(form.id)
