...
private
  ...
  def section_2_tasks
    if FeatureService.enabled?(:submission_email_confirmation)
      [{ task_name: I18n.t("forms.task_lists.section_2.set_email"), path: set_form_email_path(@form.id) },
       { task_name: I18n.t("forms.task_lists.section_2.confirm_email"), path: confirm_form_email_path(@form.id) }]
    else
