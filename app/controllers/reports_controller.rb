class ReportsController < ApplicationController
  ...
  def forms_with_routes
    forms_feature_report(:forms_with_routes)
  end

  def forms_with_payments
    forms_feature_report(:forms_with_payments)
  end

  def forms_with_csv_submission_enabled
    forms_feature_report(:forms_with_csv_submission_enabled)
  end
...
private
  ...
  def questions_feature_report(report)
    ...
    if params[:format] == "csv"
      ...
      render template: "reports/feature_report", locals: { report:, records: questions }
    end
  end

  def forms_feature_report(report)
    forms = Reports::FormDocumentsService.live_form_documents
    forms = Reports::FeatureReportService.new(forms).send report
    ...
    if params[:format] == "csv"
      ...
      render template: "reports/feature_report", locals: { report:, records: forms }
    end
  end
