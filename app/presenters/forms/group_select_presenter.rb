class Forms::GroupSelectPresenter
  ...
  def group_name(group)
    return group.name if @form.has_live_version

    "#{group.name} (#{group.status.humanize})"
  end

  def legend
    return I18n.t("helpers.legend.forms_group_select.no_active_groups") if @form.is_live? && @groups.empty?
  ...
  def hint
    ...
    return I18n.t("helpers.hint.forms_group_select.group") if @form.is_live? && @groups.present?
