...
RSpec.describe Pages::QuestionTextInput, type: :model do
  ...
  describe "validations" do
    [nil, ""].each do |question_text|
      it "is invalid given {question_text} question text" do
        ...
        expect(question_text_input).to be_invalid
        expect(question_text_input.errors.full_messages_for(:question_text)).to include("Question text #{error_message}")
      end
    end
    ...
    it "is invalid if question text is more than 250 characters" do
      question_text_input.question_text = "A" * 251
      expect(question_text_input).not_to be_valid
      error_message = I18n.t("activemodel.errors.models.pages/question_text_input.attributes.question_text.too_long", count: 250)
      expect(question_text_input.errors.full_messages_for(:question_text)).to include("Question text #{error_message}")
    end
  ...
  describe "#submit" do
    it "returns false if the form is invalid" do
      allow(question_text_input).to receive(:invalid?).and_return(true)
      expect(question_text_input.submit).to be_falsey
    end

    it "sets draft_question question_text" do
      question_text_input.question_text = "What is your name?"
      question_text_input.submit

      expect(question_text_input.draft_question.question_text).to eq("What is your name?")
    end
  end
end
