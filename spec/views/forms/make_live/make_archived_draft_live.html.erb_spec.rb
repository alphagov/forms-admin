...
describe "forms/make_live/make_archived_draft_live.html.erb" do
  ...
  context "when there are no errors" do
    before do
      render template: "forms/make_live/make_archived_draft_live", locals: { current_form: }
    end

    it "has the correct page title" do
      expect(view.content_for(:title)).to eq t("page_titles.make_archived_draft_live")
    end

    it "contains a heading" do
      expect(rendered).to have_css("h1", text: t("page_titles.make_archived_draft_live"))
    end

    it "contains the body text" do
      expect(rendered).to include(t("make_archived_draft_live.new.body_html", submission_email: current_form.submission_email))
    end

    it "renders radio buttons for making the archived changes live" do
  ...
  context "when there are errors" do
    ...
    it "sets the page title with error prefix" do
      expect(view.content_for(:title)).to eq(title_with_error_prefix(t("page_titles.make_archived_draft_live"), true))
    end
  end
end
