...
RSpec.describe PreviewLinkComponent::View, type: :component do
  ...
  context "when the form has pages" do
    it "renders the link" do
      render_inline(described_class.new([{ id: 183, question_text: "What is your address?", hint_text: "", answer_type: "address" }], preview_url))
      expect(page).to have_link("Preview this form", href: preview_url)
    end

    context "when link text is provided" do
      it "renders the provided text" do
        render_inline(described_class.new([{ id: 183, question_text: "What is your address?", hint_text: "", answer_type: "address" }], preview_url, "Custom link text"))
        expect(page).to have_link("Custom link text", href: preview_url)
      end
    end
  end
