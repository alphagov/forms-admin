...
describe PageOptionsService do
  ...
  let(:form) { create :form, :live, pages: }
  ...
  describe "#all_options_for_answer_type" do
    ...
    context "with conditions" do
      let(:page) { create :page, id: 1, answer_type: "email", routing_conditions: }
      let(:condition_pointing_to_page_3) { create :condition, routing_page_id: 1, check_page_id: 1, answer_value: "Wales", goto_page_id: 3 } # rubocop:disable RSpec/IndexedLet
      let(:condition_pointing_to_page_4) { create :condition, routing_page_id: 1, check_page_id: 1, answer_value: "England", goto_page_id: 4 } # rubocop:disable RSpec/IndexedLet
      let(:routing_conditions) { [] }

      context "with a legacy page that doesn't have routing conditions method" do
        subject(:page_options_service) do
          page_without_routing_conditions_method = page.dup.tap { |p| p.routing_conditions = [] }
