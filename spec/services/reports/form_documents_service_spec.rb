...
RSpec.describe Reports::FormDocumentsService do
  ...
  let(:branch_route_form) { form_documents[2] }
  let(:skip_route_form) { form_documents[3] }

  let(:branch_route_form_2x1) do
    form_document = branch_route_form.deep_dup
    form_document["content"]["steps"] = [
      *form_document["content"]["steps"].deep_dup.each do |step|
        step["id"] += 100
        step["next_step_id"] += 100 if step["next_step_id"]
        step["routing_conditions"].each do |condition|
          condition["id"] += 100
          condition["goto_page_id"] += 100 if condition["goto_page_id"]
          condition["check_page_id"] += 100 if condition["check_page_id"]
          condition["routing_page_id"] += 100 if condition["routing_page_id"]
        end
      end,
      *form_document["content"]["steps"].deep_dup.each do |step|
        step["id"] += 200
        step["next_step_id"] += 200 if step["next_step_id"]
        step["routing_conditions"].each do |condition|
          condition["id"] += 200
          condition["goto_page_id"] += 200 if condition["goto_page_id"]
          condition["check_page_id"] += 200 if condition["check_page_id"]
          condition["routing_page_id"] += 200 if condition["routing_page_id"]
        end
      end,
    ]
    form_document
  ...
  describe ".form_documents" do
    ...
    context "when there are forms from internal organisations" do
      ...
      it "does not include these forms in the draft_form_documents output" do
        form_documents = described_class.form_documents(tag: "live").to_a
        expect(form_documents.size).to eq(9)
  ...
  describe ".has_secondary_skip_routes?" do
    ...
    context "when form has two steps each with one secondary skip condition" do
      let(:form_document) { branch_route_form_2x1 }

      it { is_expected.to be true }
    end

    context "when form has no secondary skip conditions" do
      let(:form_document) { form_documents[3] }
