...
RSpec.describe FormCopyService do
  ...
  describe "#copy" do
    ...
    context "transactionality when Welsh copy fails" do
      let(:source_form) do
      ...
      it "rolls back the entire copy if Welsh translation copying fails" do
        ...
        allow_any_instance_of(FormCopyService).to receive(:copy_welsh_translations)
          .and_raise(ActiveRecord::RecordInvalid.new(Form.new))

        expect {
          described_class.new(source_form, logged_in_user).copy(tag: "live")
