...
feature "Copy a form", type: :feature do
  ...
  def then_i_am_shown_the_copy_form_page
    expect(page.find("h1")).to have_text "Name"
    expect(page).to have_field("Name", with: "Copy of #{original_form.name}")
    expect_page_to_have_no_axe_errors(page)
  end

  def when_i_enter_a_new_name_for_the_copied_form
    fill_in "Name", with: "My copied juggling license form"
  ...
  def and_i_am_redirected_to_the_copied_form
    ...
    # Verify the copied form exists in the database
    copied_form_id = current_path.split("/").last.to_i
    copied_form = Form.find(copied_form_id)
    expect(copied_form.id).not_to eq(original_form.id)
    expect(copied_form.pages.count).to eq(original_form.pages.count)
  end
